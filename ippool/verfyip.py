...
class TryIp(object):
    ...
    def add_ip_from_newproxyqueue(self):
        ...
        while True:
            ...
            if ip:
                self.con.lpush('new_ip',ip)
                print(ip)
                self.new_proxy_queue.task_done()

    @retry(stop_max_attempt_number=3)
    def _parse_url(self,proxy):
        response = requests.get(self.url, headers=self.headers,proxies=proxy,timeout=5)
